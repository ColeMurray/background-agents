...
def require_secret() -> str:
    """
    Get the DAYTONA_API_SECRET, raising an error if not configured.
    ...
    Raises:
        AuthConfigurationError: If DAYTONA_API_SECRET is not set
    """
    secret = os.environ.get("DAYTONA_API_SECRET")
    if not secret:
        raise AuthConfigurationError(
            "DAYTONA_API_SECRET environment variable is not configured. "
            "This secret is required for authenticating control plane requests. "
            "Please ensure the DAYTONA_API_SECRET environment variable is properly set."
        )
    return secret
...
def generate_internal_token(secret: str | None = None) -> str:
    ...
    Args:
        secret: The shared secret for HMAC signing. If not provided, reads from
                DAYTONA_API_SECRET environment variable.

    Returns:
        A token string in the format "timestamp.signature"

    Raises:
        AuthConfigurationError: If secret is not provided and DAYTONA_API_SECRET is not set
    """
    if secret is None:
