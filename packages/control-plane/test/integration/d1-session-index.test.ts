...
describe("D1 SessionIndexStore", () => {
  ...
  it("backfills automation lineage on legacy descendants", async () => {
    const store = new SessionIndexStore(env.DB);
    const now = Date.now();
    const baseSession = {
      title: null,
    ...
    await store.create({
      ...
      automationRunId: "run-legacy",
    });
    await store.create({
      ...baseSession,
      id: "legacy-automation-child",
      parentSessionId: "legacy-automation-root",
      spawnSource: "agent",
    });
    await store.create({
      ...baseSession,
      id: "legacy-automation-grandchild",
      parentSessionId: "legacy-automation-child",
      spawnSource: "agent",
    });

    const migration = env.TEST_MIGRATIONS.find((item) =>
      item.name.startsWith("0054_inherit_automation_session_lineage")
    );
    const backfillQuery = migration?.queries.find((query) =>
      query.includes("WITH RECURSIVE automation_descendants")
    );
    expect(backfillQuery).toBeDefined();
    await env.DB.prepare(backfillQuery!).run();

    for (const id of ["legacy-automation-child", "legacy-automation-grandchild"]) {
      const session = await store.get(id);
      expect(session?.automationId).toBe("automation-legacy");
      expect(session?.automationRunId).toBe("run-legacy");
    }
  });
});
