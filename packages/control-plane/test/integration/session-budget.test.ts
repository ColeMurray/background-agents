...
describe("session budgets", () => {
  ...
  it("allows only the owner to change the live limit through the public route", async () => {
    ...
    const memberResponse = await stub.fetch("http://internal/internal/budget", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId: "member-user", maxCostUsd: null }),
    });
  ...
  it("warns, exhausts active work, preserves pending work, and clears on a raised limit", async () => {
    ...
    expect((await sendCost(7)).status).toBe(200);
    expect(await queryDO(stub, "SELECT id FROM events WHERE type = 'warning'")).toEqual([]);
    expect((await sendCost(1)).status).toBe(200);
    expect((await sendCost(2)).status).toBe(200);
    ...
    const raised = await stub.fetch("http://internal/internal/budget", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId: "user-1", maxCostUsd: 20 }),
    });
