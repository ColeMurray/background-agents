...
describe("ImageBuildStore finalization state", () => {
  ...
  it("fences completion to the base release captured before launch", async () => {
    ...
    await store.registerBuild({
      ...
      baseRelease: {
        schemaVersion: 1,
        baseReleaseId: "c".repeat(64),
        artifact: { provider: "e2b", scope: "account", reference: "template-1" },
        identity: {
          target: "e2b",
    ...
    await store.bindProviderSession("identity-build", "e2b", "session");
    const completion = {
    ...
    expect(await store.finalization.acceptSuccessfulCompletion(completion)).toBe("rejected");
    const metadata = {
    ...
    expect(
      await store.finalization.acceptSuccessfulCompletion({
        ...completion,
        ...metadata,
        baseRecipeDigest: "d".repeat(64),
      })
    ).toBe("rejected");
    expect(
      await store.finalization.acceptSuccessfulCompletion({ ...completion, ...metadata })
    ).toBe("accepted");
    expect((await store.getStatus(scope))[0]).toMatchObject({
      baseReleaseId: "c".repeat(64),
      baseRecipeDigest: "a".repeat(64),
      baseInventoryDigest: "b".repeat(64),
      imageTarget: "e2b",
