...
/**
 * What every host must satisfy. Storage contracts are the repository suites
 * registered below, run by each host against its own session storage. Host
 * contracts are behaviors of the live runtime (claiming, sockets) that a
 * storage factory cannot exercise; each names the integration test that
 * implements it on the Cloudflare host, and a Node host must provide the same
 * test under the same title.
 */
export const SESSION_CORE_CONFORMANCE_MANIFEST = [
  { id: "repository.session-core", scope: "storage" },
  { id: "repository.message", scope: "storage" },
  { id: "repository.event", scope: "storage" },
  { id: "repository.sandbox", scope: "storage" },
  { id: "repository.participant", scope: "storage" },
  { id: "repository.artifact", scope: "storage" },
  { id: "repository.session-attachment", scope: "storage" },
  { id: "repository.ws-client-mapping", scope: "storage" },
  {
    id: "host.concurrent-prompt-claim",
    scope: "host",
    test: "test/integration/prompt-enqueue.test.ts",
    title: "dispatches exactly one of two concurrent prompts and leaves the other queued",
  },
  {
    id: "host.socket-terminal-upgrade",
    scope: "host",
    test: "test/integration/websocket-sandbox.test.ts",
    title: "revalidates terminal state after asynchronous authentication",
  },
  {
    id: "host.socket-single-sandbox",
    scope: "host",
    test: "test/integration/websocket-sandbox.test.ts",
    title: "keeps exactly one sandbox socket and tags it with the persisted sandbox ID",
  },
  {
    id: "host.socket-ack-redelivery",
    scope: "host",
    test: "test/integration/websocket-sandbox.test.ts",
  ...
  describe("session-core repository conformance", () => {
    it("persists session and member repository updates", async () => {
      await storageFactory(({ sql, transactionSync }) => {
        ...
        expect(repository.updateSessionTitleIfUnset("conformance-session", "Generated", 200)).toBe(
          true
        );
  ...
  describe("session attachment repository conformance", () => {
    it("claims and releases attachments using rowsWritten", async () => {
      await storageFactory(({ sql }) => {
        ...
        expect(() => repository.claimForMessage("message-2", ["attachment-1", "missing"])).toThrow(
          AttachmentClaimConflictError
        );
      });
    });
