...
describe("ImageBuildWorkflow", () => {
  ...
  describe("provider-session build callbacks", () => {
    ...
    it("publishes failures before atomically accepting them", async () => {
      const store = sessionBuildStore();
      const queue = { send: vi.fn().mockResolvedValue(undefined) };
      const { workflow } = createWorkflow({ store, queue });
      ...
      expect(queue.send.mock.invocationCallOrder[0]).toBeLessThan(
        store.acceptFailedCompletion.mock.invocationCallOrder[0]
      );
    });
