...
/**
 * Generate an installation token (alias for getCachedInstallationToken).
 * Used by session DO for git push; accepts optional env for KV cache.
 */
export async function generateInstallationToken(
  config: GitHubAppConfig,
  env?: InstallationTokenCacheBindings
): Promise<string> {
  return getCachedInstallationToken(config, env);
}
