...
function fixture(
  ...
  const recordSandboxSnapshot = vi.fn(() => true);
  const shutdown = new SandboxShutdownCoordinator({
    ...
    session: {
      getSession: () => ({
        id: "session-1",
        session_name: "shutdown-safety",
        sandbox_execution: options.sandboxExecution,
      }),
    },
  ...
  return { shutdown, provider, sandboxRow, stopSandbox, store, recordSandboxSnapshot };
}

describe("sandbox shutdown safety", () => {
  it("fails closed on malformed execution metadata before checkpoint capture or publication", async () => {
    const takeSnapshot = vi.fn(async () => ({
      success: true,
      imageId: "must-not-publish",
      sourceStopped: false,
    }));
    const h = fixture(runningRecord(), {
      takeSnapshot,
      sandboxExecution: '{"profile":"docker-v1","provider":"modal"}',
    });
    ...
    expect(h.recordSandboxSnapshot).not.toHaveBeenCalled();
    expect(h.shutdown.admissionDecision()).toBe("held");
    expect(h.store.value).toMatchObject({ phase: "failed" });
  });

  it("carries the saved execution profile into the recovery decision", () => {
    const h = fixture(
      recoveryRecord({
        phase: "saved",
        sourceRetired: true,
    ...
    expect(h.shutdown.startupDecision()).toMatchObject({
      kind: "restore_snapshot",
      snapshotId: "docker-snapshot",
      executionProfile: "docker-v1",
    });
  });
