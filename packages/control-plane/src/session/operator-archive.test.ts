...
describe("archiveOperatorSessionPage", () => {
  ...
  it("advances past an index row whose runtime is gone instead of replaying it", async () => {
    const index: OperatorArchiveIndex = {
      listOperatorArchiveCandidates: vi.fn().mockResolvedValue({
        candidates: [{ id: "orphan", createdAt: 7, indexStatus: "active" }],
        hasMore: true,
      }),
    };
    const runtime = {
      fetch: vi.fn(async () => Response.json({ error: "Session not found" }, { status: 404 })),
    } as unknown as SessionRuntimeClient;
    ...
    expect(result.nextCursor).toBe("100:7:orphan");
  });
