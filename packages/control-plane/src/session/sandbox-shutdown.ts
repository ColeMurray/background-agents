...
export class SandboxShutdownCoordinator {
  ...
  async captureCheckpoint(
    ...
    try {
      ...
      this.fail(state, "failed", "Invalid sandbox execution metadata blocks checkpointing.");
      return { outcome: "held" };
    }
  ...
  private async capture(state: ShutdownRecord): Promise<void> {
    ...
    const session = this.deps.session.getSession();
    if (!session) {
      this.fail(state, "failed", "Session state is unavailable for final snapshot.");
      return;
    }
    let executionProfile: "default" | "docker-v1";
    try {
  ...
  private executionProfileForProvider(rawExecution: string | null): "default" | "docker-v1" {
    const execution = parseSessionSandboxExecution(rawExecution);
    if (execution.profile === "docker-v1" && execution.provider !== this.deps.provider.name) {
      throw new Error("Docker execution is bound to another sandbox provider");
    }
