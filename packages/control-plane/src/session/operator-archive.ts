...
export interface OperatorArchiveBatchResult {
  ...
  /**
   * Index rows whose Durable Object no longer exists while the row still
   * claims a live status. These are reported separately rather than retried:
   * retrying an orphaned row would
   * pin the cursor to this page forever and starve every later session.
   */
  orphanedIds: string[];
...
export function parseOperatorArchiveCursor(
  ...
  if (!DECIMAL_CURSOR_FIELD.test(parts[0]) || !DECIMAL_CURSOR_FIELD.test(parts[1])) {
...
export async function archiveOperatorSessionPage(options: {
    ...
    switch (outcome.value) {
      ...
      case "missing":
        if (candidate.indexStatus === "archived") {
          ...
          result.orphanedIds.push(sessionId);
        }
        break;
      default: {
  ...
  // A failure is retryable, so the page is replayed from the same cursor until
  // it clears. Only outcomes that can change on a retry may pin the cursor.
  ...
  log.info("Operator session archive page completed", {
    ...
    orphaned_ids: result.orphanedIds,
