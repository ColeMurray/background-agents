...
describe("SessionStatusService transition modelling", () => {
  // The table is documentation first and a runtime signal second. It warns
  // rather than throws on purpose: rejecting a transition the code really
  // performs is far more damaging than failing to model one, and this table
  // was hand-derived. See reportUnmodelledTransition for the full reasoning.
  it("warns but still writes when a transition is not modelled", async () => {
    const h = harness({ session: createSession({ status: "completed" }) });

    const changed = await h.service.transition("failed");
    ...
    expect(h.log.warn).toHaveBeenCalledWith(
      "session.transition.unmodelled",
      expect.objectContaining({ from: "completed", to: "failed" })
    );
  });
  ...
  it.each([
    ...
    ["archived", "completed"],
    ["completed", "active"],
  ] as const)("stays silent for the modelled transition %s -> %s", async (from, to) => {
    const h = harness({ session: createSession({ status: from }) });

    await h.service.transition(to);

    expect(h.log.warn).not.toHaveBeenCalledWith("session.transition.unmodelled", expect.anything());
  });
});
