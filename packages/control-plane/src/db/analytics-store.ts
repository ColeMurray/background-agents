...
/**
 * PR analytics deliberately has no spawn-source filter (automation-produced
 * PRs are output too); `now` anchors the open-inventory age computation.
 */
...
interface PullRequestFunnelRow {
  created: number;
  open: number;
  draft: number;
  merged: number;
...
interface PullRequestInventoryRow {
  total: number;
  avg_age_ms: number | null;
}

interface PullRequestDailyCountRow {
  date: string;
  count: number;
}

interface PullRequestRepoRow {
  key: string;
  created: number;
  merged: number;
  closed: number;
...
interface PullRequestSourceRow {
  source: string;
  created: number;
  merged: number;
}
...
export class AnalyticsStore {
  ...
  /**
   * PR-scoped analytics over session_pull_requests (docs/pr-analytics-design.md).
   *
  ...
  async getPullRequests(
    filters: PullRequestAnalyticsFilters
  ): Promise<AnalyticsPullRequestsResponse> {
    ...
    const inventory = await this.db
      ...
      .bind(filters.now)
      .first<PullRequestInventoryRow>();

    const createdByDay = await this.db
      .prepare(
      ...
      .bind(filters.startAt, filters.endAt)
      .all<PullRequestDailyCountRow>();

    const mergedByDay = await this.db
      .prepare(
