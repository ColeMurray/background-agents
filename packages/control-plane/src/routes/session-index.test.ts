...
describe("session index routes", () => {
  ...
  it("requires a human user for read-state mutations", async () => {
    const response = await patchReadState(JSON.stringify({ action: "mark_read" }), {
      kind: "service",
      service: "linear-bot",
      actor: null,
    });
  ...
  it("requires a session ID for read-state mutations", async () => {
    ...
    const response = await patchReadState(
      JSON.stringify({ action: "mark_read" }),
  ...
  it.each([
    ["invalid JSON", "{"],
    ["an invalid action", JSON.stringify({ action: "mark_read", userId: "user-2" })],
  ])("rejects %s for read-state mutations", async (_description, body) => {
    const response = await patchReadState(body, { kind: "user", userId: "user-1" });
  ...
  it.each([
    [JSON.stringify({ action: "mark_read" }), { kind: "mark_read" }],
    [
      JSON.stringify({ action: "acknowledge", observedAttentionId: "message-1" }),
      { kind: "acknowledge", observedAttentionId: "message-1" },
    ],
