...
// ==================== E2B-style pause/resume + runtime-cap hooks ====================

/**
 * Provider mock shaped like the E2B backend: explicit stop + persistent resume,
 * with the optional pauseSandbox / onUserActivity / shouldResetRuntime hooks.
 */
function createPauseResumeProvider(
  overrides: Partial<{
    pauseSandbox: (c: StopConfig) => Promise<StopResult>;
    stopSandbox: (c: StopConfig) => Promise<StopResult>;
    resumeSandbox: (c: ResumeConfig) => Promise<ResumeResult>;
    onUserActivity: (c: { providerObjectId: string; sessionId: string }) => Promise<void>;
    shouldResetRuntime: (c: {
  ...
  return {
    ...
    stopSandbox: vi.fn(overrides.stopSandbox ?? (async () => ({ success: true }))),
    pauseSandbox: vi.fn(overrides.pauseSandbox ?? (async () => ({ success: true }))),
    resumeSandbox: vi.fn(
      overrides.resumeSandbox ??
        (async (c: ResumeConfig) => ({ success: true, providerObjectId: c.providerObjectId }))
    ),
    onUserActivity: vi.fn(overrides.onUserActivity ?? (async () => {})),
    shouldResetRuntime: vi.fn(
      overrides.shouldResetRuntime ?? (async () => ({ shouldReset: false }))
    ),
  };
}

describe("SandboxLifecycleManager — E2B pause/resume + runtime cap", () => {
  function makeManager(
    ...
    const storage = createMockStorage(createMockSession(), sandbox as never);
    const broadcaster = createMockBroadcaster();
    const wsManager = createMockWebSocketManager(false, 0);
    const alarmScheduler = createMockAlarmScheduler();
    // Captures detached best-effort work (provider TTL refreshes) so tests can
    // await it explicitly — mirrors the DO wiring ctx.waitUntil.
  ...
  it("pauses (not kills) on inactivity timeout when the provider supports pause", async () => {
    const now = Date.now();
    const provider = createPauseResumeProvider();
    const sandbox = createMockSandbox({
      status: "ready",
      last_heartbeat: now - 10000,
      last_activity: now - 11 * 60 * 1000,
    });
    const { manager } = makeManager(provider, sandbox);

    await manager.handleAlarm();

    expect(provider.pauseSandbox).toHaveBeenCalledTimes(1);
    expect(provider.pauseSandbox).toHaveBeenCalledWith(
      expect.objectContaining({ reason: "inactivity_timeout", providerObjectId: "modal-obj-123" })
    );
    expect(provider.stopSandbox).not.toHaveBeenCalled();
  });

  it("pauses (not kills) on heartbeat-stale when the provider supports pause", async () => {
    const now = Date.now();
    const provider = createPauseResumeProvider();
    const sandbox = createMockSandbox({ status: "ready", last_heartbeat: now - 100000 });
    const { manager } = makeManager(provider, sandbox);
  ...
  it("kills (not pauses) on connecting timeout — no resume value", async () => {
    const now = Date.now();
    const provider = createPauseResumeProvider();
    const sandbox = createMockSandbox({
      status: "connecting",
      created_at: now - 10 * 60 * 1000, // past connecting timeout
    });
    const { manager } = makeManager(provider, sandbox);
    ...
    expect(provider.pauseSandbox).not.toHaveBeenCalled();
  });

  it("falls through to stopSandbox on inactivity when provider has no pause (Daytona)", async () => {
    const now = Date.now();
    const stopSandbox = vi.fn(async () => ({ success: true }));
    // Daytona-shaped provider: explicit stop, persistent resume, NO pauseSandbox.
    const provider: SandboxProvider = {
      name: "daytona-like",
      capabilities: {
        supportsSnapshots: false,
      ...
      createSandbox: vi.fn(async (c: CreateSandboxConfig) => ({
    ...
    expect(stopSandbox).toHaveBeenCalledWith(
      expect.objectContaining({ reason: "inactivity_timeout" })
    );
  });

  it("pause+resume-cycles the sandbox when shouldResetRuntime returns true", async () => {
    const now = Date.now();
    const provider = createPauseResumeProvider({
      shouldResetRuntime: async () => ({ shouldReset: true }),
    });
    // Healthy sandbox: recent heartbeat + activity so only the runtime-cap path fires.
    const sandbox = createMockSandbox({
      status: "ready",
    ...
    const { manager, storage } = makeManager(provider, sandbox);

    await manager.handleAlarm();

    expect(provider.shouldResetRuntime).toHaveBeenCalled();
    expect(provider.pauseSandbox).toHaveBeenCalledWith(
      expect.objectContaining({ reason: "runtime_cap_reset" })
    );
    expect(provider.resumeSandbox).toHaveBeenCalledTimes(1);
    // Active sandbox — the inactivity branch must not have stopped it.
    expect(storage.calls).not.toContain("updateSandboxStatus:stopped");
  });

  it("pauses for inactivity instead of cycling when the sandbox is idle AND past the cap", async () => {
    const now = Date.now();
    const shouldResetRuntime = vi.fn(async () => ({ shouldReset: true }));
    const provider = createPauseResumeProvider({ shouldResetRuntime });
    // Idle past the inactivity timeout AND past the runtime cap: the idle
    // disposition must win — cycling would keep an abandoned sandbox alive
    // (and billable) forever, since the post-resume bridge reconnect refreshes
    // last_activity.
    const sandbox = createMockSandbox({
      status: "ready",
      last_heartbeat: now - 5000,
    ...
    expect(shouldResetRuntime).not.toHaveBeenCalled();
  });

  it("defers the runtime-cap cycle while a message is executing", async () => {
    const now = Date.now();
    const shouldResetRuntime = vi.fn(async () => ({ shouldReset: true }));
    const provider = createPauseResumeProvider({ shouldResetRuntime });
    const sandbox = createMockSandbox({
    ...
    const { manager, alarmScheduler } = makeManager(provider, sandbox, {
      getIsProcessing: () => true,
    });

    await manager.handleAlarm();

    // Skipped mid-execution; the next alarm (still scheduled) retries.
    expect(shouldResetRuntime).not.toHaveBeenCalled();
    expect(provider.pauseSandbox).not.toHaveBeenCalled();
    expect(alarmScheduler.alarms.length).toBe(1);
  });

  it("keeps the alarm chain alive after a runtime-cap cycle", async () => {
    const now = Date.now();
    const provider = createPauseResumeProvider({
      shouldResetRuntime: async () => ({ shouldReset: true }),
    });
    ...
    expect(provider.resumeSandbox).toHaveBeenCalledTimes(1);
    // The cap cycle must not orphan the lifecycle: a follow-up alarm is
    // scheduled even if the bridge never reconnects after the cycle.
    expect(alarmScheduler.alarms.length).toBeGreaterThanOrEqual(1);
  });

  it("clears code-server/tunnel state when a resume attempt fails", async () => {
    const provider = createPauseResumeProvider({
      resumeSandbox: vi.fn(async () => {
        throw new Error("E2B 502");
      }),
    });
    const sandbox = createMockSandbox({ status: "stopped" });
    const { manager, storage } = makeManager(provider, sandbox);

    await manager.spawnSandbox(); // resume branch (persistent resume + providerObjectId)

    expect(provider.resumeSandbox).toHaveBeenCalled();
    expect(storage.calls).toContain("updateSandboxStatus:failed");
    // Clients must not be left holding dead URLs for an unreachable sandbox.
    expect(storage.calls).toContain("clearSandboxTunnelUrls");
  });

  it("does not reset runtime when shouldResetRuntime returns false", async () => {
    const now = Date.now();
    const provider = createPauseResumeProvider({
      shouldResetRuntime: async () => ({ shouldReset: false }),
    });
    const sandbox = createMockSandbox({
      status: "ready",
      last_heartbeat: now - 5000,
  ...
  it("leaves handleAlarm unchanged for providers without shouldResetRuntime (Modal/Daytona)", async () => {
    const now = Date.now();
    // Default mock provider has no pauseSandbox/shouldResetRuntime and is snapshot-based.
    const provider = createMockProvider();
    const sandbox = createMockSandbox({
      status: "ready",
      last_heartbeat: now - 5000,
      last_activity: now - 5000,
      created_at: now - 90 * 60 * 1000, // very old — must NOT trigger any reset
    });
    const { manager, alarmScheduler, storage } = makeManager(provider, sandbox);

    await manager.handleAlarm();

    // Healthy + active → just schedules the next routine alarm, nothing else.
    expect(alarmScheduler.alarms.length).toBe(1);
    expect(storage.calls).not.toContain("updateSandboxStatus:stopped");
    expect(storage.calls).not.toContain("updateSandboxStatus:stale");
  });

  it("refreshes provider TTL on activity via a detached onUserActivity call", async () => {
    const provider = createPauseResumeProvider();
    const sandbox = createMockSandbox({ status: "ready" });
    const { manager, storage, flushDetached } = makeManager(provider, sandbox);

    manager.updateLastActivity(Date.now());

    // The storage write is synchronous; the provider refresh runs detached.
    expect(storage.calls).toContain("updateSandboxLastActivity");
    await flushDetached();
    expect(provider.onUserActivity).toHaveBeenCalledWith(
      expect.objectContaining({ providerObjectId: "modal-obj-123" })
    );
  });

  it("does not fail the activity path when onUserActivity throws", async () => {
    const provider = createPauseResumeProvider({
    ...
    expect(() => manager.updateLastActivity(Date.now())).not.toThrow();
    // The detached refresh swallows the error — flushing must not reject.
    await expect(flushDetached()).resolves.toBeDefined();
    expect(storage.calls).toContain("updateSandboxLastActivity");
  });

  it("skips onUserActivity for providers that don't implement it (Modal/Daytona)", async () => {
    const provider = createMockProvider(); // no onUserActivity
    const sandbox = createMockSandbox({ status: "ready" });
    const { manager, storage, flushDetached } = makeManager(provider, sandbox);
    ...
    expect(storage.calls).toContain("updateSandboxLastActivity");
    // Nothing was detached — no provider hook to call.
    await expect(flushDetached()).resolves.toHaveLength(0);
  });
});
