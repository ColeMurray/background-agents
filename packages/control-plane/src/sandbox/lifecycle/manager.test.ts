...
describe("SandboxLifecycleManager", () => {
  ...
  describe("terminateSandboxForRevocation", () => {
    function revocationManager(overrides: Parameters<typeof createMockProvider>[0] = {}) {
      const storage = createMockStorage();
      const wsManager = createMockWebSocketManager(true);
      const manager = new SandboxLifecycleManager(
      ...
      return { manager, storage, wsManager };
    }

    it("reports terminated only after the provider confirmed the stop", async () => {
      const stopSandbox = vi.fn(async () => ({ success: true }));
      const { manager, storage, wsManager } = revocationManager({
        capabilities: { supportsExplicitStop: true },
        stopSandbox,
      });

      await expect(
        manager.terminateSandboxForRevocation("provider account disabled")
      ).resolves.toBe("terminated");

      expect(storage.calls).toContain("updateSandboxStatus:failed");
      expect(stopSandbox).toHaveBeenCalledWith(
        expect.objectContaining({ reason: "credential_revoked" })
      );
      expect(wsManager.detachSandboxWebSocket).toHaveBeenCalledWith(
        1011,
    ...
    it("propagates a provider stop failure so the revocation is retried", async () => {
      const { manager } = revocationManager({
        capabilities: { supportsExplicitStop: true },
        stopSandbox: vi.fn(async () => ({ success: false, error: "provider unavailable" })),
      });
    ...
    it("only requests a shutdown when the provider cannot stop a sandbox on demand", async () => {
      const { manager, wsManager } = revocationManager({
        capabilities: { supportsExplicitStop: false },
      });

      await expect(
        manager.terminateSandboxForRevocation("provider account disabled")
      ).resolves.toBe("shutdown_requested");
      expect(wsManager.sendToSandbox).toHaveBeenCalledWith({ type: "shutdown" });
      expect(wsManager.detachSandboxWebSocket).toHaveBeenCalledWith(
        1011,
        "Provider credential revoked"
    ...
    it.each(["stopped", "stale", "failed"] as const)(
      "reports a %s sandbox as gone",
      async (status) => {
        const storage = createMockStorage(createMockSession(), createMockSandbox({ status }));
        const wsManager = createMockWebSocketManager(true);
        const manager = new SandboxLifecycleManager(
          createMockProvider({ capabilities: { supportsExplicitStop: true } }),
          storage,
          storage,
          createMockBroadcaster(),
          wsManager,
