...
/**
 * Provider-neutral inputs shared by fresh launches and snapshot restores.
 * Launch identity and image selection belong to the operation-specific config.
 */
export interface SandboxLaunchInputs {
  /** Repository owner */
  ...
  vncEnabled?: boolean;
  /**
   * Whether to install the agent-initiated slack-notify tool. Resolved fresh
   * on each launch or restore. Fixed for the lifetime of the sandbox;
   * per-call authorization is re-evaluated by the control-plane endpoint.
   */
  ...
  /**
   * Ordered member list for multi-repo or pinned sessions. Only set when the
   * session has more than one member or a pinned base SHA. Unpinned single-repo
   * sessions keep the scalar repoOwner/repoName/branch wire form (the runtime synthesizes its
...
export interface CreateSandboxConfig extends SandboxLaunchInputs {
...
/**
 * Access artifacts returned by a fresh launch or snapshot restore.
 */
export interface SandboxStartupAccess {
...
export interface CreateSandboxResult extends SandboxStartupAccess {
 ...
 */
export interface RestoreConfig extends SandboxLaunchInputs {
 ...
 */
export interface RestoreResult extends SandboxStartupAccess {
