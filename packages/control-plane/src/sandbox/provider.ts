...
export interface CreateSandboxConfig {
  ...
  sandboxId: string;
  /** Provider-safe durable idempotency key for VM allocation recovery. */
...
export interface ReconcileAllocationConfig {
  allocationName: string;
  sessionId: string;
  sandboxId: string;
}

export interface TerminateAllocationConfig {
  allocationName: string;
  providerObjectId: string;
  sessionId: string;
  sandboxId: string;
...
/** Definitive absence of a session filesystem, never a fresh-spawn signal. */
export class SnapshotArtifactUnavailableError extends SandboxProviderError {
  constructor(message: string, cause?: Error) {
    super(message, "permanent", cause);
...
export class InvalidAllocationResponseError extends SandboxProviderError {
  constructor(
    public readonly providerObjectId: string | undefined,
    cause?: Error
  ) {
    super("Provider allocation response is invalid", "permanent", cause);
    this.name = "InvalidAllocationResponseError";
  }
}
...
export interface SandboxProvider {
  ...
  /** Resolve a deterministic allocation name. Null means unknown, not terminal absence. */
  reconcileAllocation?(config: ReconcileAllocationConfig): Promise<string | null>;
  /** Terminate only after the provider verifies the exact allocation owner. */
  terminateAllocation?(config: TerminateAllocationConfig): Promise<void>;
}
