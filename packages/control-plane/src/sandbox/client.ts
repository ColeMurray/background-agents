        ...
        try {
          if ("buildId" in owner) {
            ...
            await this.postJson(
              ...
              "terminateInvalidAllocation",
              MODAL_CLEANUP_REQUEST_DEADLINE_MS,
              { provider_object_id: id, session_id: owner.sessionId, sandbox_id: owner.sandboxId },
          ...
          throw new Error(`Modal allocation validation failed; cleanup required for ${id}`);
        }
      } else {
        log.error("modal.ambiguous_allocation", ownerLogFields);
      }
      throw new Error("Modal VM allocation response is invalid or has the wrong execution profile");
    };
  }
