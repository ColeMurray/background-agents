...
const reconcileSandboxAllocationResponseSchema = z.object({
  success: z.literal(true),
  data: z.discriminatedUnion("state", [
    z.object({ state: z.literal("unknown") }),
    z.object({ state: z.literal("running"), provider_object_id: z.string().min(1) }),
  ]),
});
...
export class ModalAllocationValidationError extends Error {
  constructor(public readonly providerObjectId?: string) {
    super("Modal VM allocation response is invalid or has the wrong execution profile");
    this.name = "ModalAllocationValidationError";
  }
}
...
export class ModalClient {
  ...
  private readonly endpointUrl: (name: string) => string;
  ...
  private async postJson<T>(
    ...
    validateAllocation?: (raw: unknown, valid: boolean) => Promise<void>
  ): Promise<T> {
    ...
    return withRequestDeadline("Modal", endpoint, deadlineMs, callerSignal, async (signal) => {
      ...
      try {
        ...
        // or malformed. Record the owner as an ambiguous cleanup obligation.
        if (validateAllocation) await validateAllocation(undefined, false);
        throw error;
      }
      if (validateAllocation) await validateAllocation(raw, schema.safeParse(raw).success);
      return parseModalApiResponse(schema, raw);
    });
  }
  ...
  private executionBody(execution: SessionSandboxExecution | undefined): Record<string, unknown> {
    if (execution === undefined) return {};
    const parsed = sessionSandboxExecutionSchema.parse(execution);
    return parsed.profile === "docker-v1" ? { sandbox_execution: parsed } : {};
  }

  async reconcileSandboxAllocation(input: {
    allocationName: string;
    sessionId: string;
    sandboxId: string;
  }): Promise<string | null> {
    const response = await this.postJson(
      this.endpointUrl("api-reconcile-sandbox-allocation"),
      "reconcileSandboxAllocation",
      MODAL_CLEANUP_REQUEST_DEADLINE_MS,
      {
        allocation_name: input.allocationName,
        session_id: input.sessionId,
        sandbox_id: input.sandboxId,
      },
      reconcileSandboxAllocationResponseSchema,
      undefined,
      undefined,
      () => {}
    );
    return response.data.state === "running" ? response.data.provider_object_id : null;
  }

  async terminateSandboxAllocation(input: {
    allocationName: string;
    providerObjectId: string;
    sessionId: string;
    sandboxId: string;
    ...
    await this.postJson(
      this.endpointUrl("api-terminate-sandbox"),
      "terminateSandboxAllocation",
      MODAL_CLEANUP_REQUEST_DEADLINE_MS,
      {
  ...
  private allocationValidator(
    execution: SessionSandboxExecution | undefined,
    owner: { sessionId: string; sandboxId?: string } | { buildId: string }
  ): ((raw: unknown, valid: boolean) => Promise<void>) | undefined {
    if (execution?.profile !== "docker-v1") return undefined;
    if ("sessionId" in owner && !owner.sandboxId) {
      throw new Error("Modal VM allocation requires a control-plane sandbox generation");
    }
    // Callers pass full requests structurally; never log their secret-bearing extras.
    const ownerLogFields =
      "buildId" in owner
        ? { build_id: owner.buildId }
        : { session_id: owner.sessionId, sandbox_id: owner.sandboxId };
    return async (raw, valid) => {
      const allocation = z
        .object({
          data: z.object({
            execution_profile: z.unknown().optional(),
            modal_object_id: z.unknown().optional(),
            provider_session_id: z.unknown().optional(),
            sandbox_id: z.unknown().optional(),
          }),
        })
        .safeParse(raw);
      const id = allocation.success
        ? typeof allocation.data.data.modal_object_id === "string"
          ? allocation.data.data.modal_object_id
          : allocation.data.data.provider_session_id
        : undefined;
      if (
        valid &&
        allocation.success &&
        allocation.data.data.execution_profile === "docker-v1" &&
        typeof id === "string" &&
        /^sb-[a-zA-Z0-9]+$/.test(id) &&
        ("buildId" in owner || allocation.data.data.sandbox_id === owner.sandboxId)
      )
        return;
      if (typeof id === "string" && /^sb-[a-zA-Z0-9]+$/.test(id)) {
        if ("buildId" in owner) {
          try {
            await this.terminateImageBuildSandbox({
              buildId: owner.buildId,
              providerSessionId: id,
              reason: "invalid VM allocation response",
            });
          } catch {
            log.error("modal.allocation_cleanup_required", {
        ...
        log.error("modal.ambiguous_allocation", ownerLogFields);
      }
      throw new ModalAllocationValidationError(typeof id === "string" ? id : undefined);
    };
  }
  ...
  async deleteProviderImage(
    ...
    await this.postJson(
      this.endpointUrl("api-delete-image"),
      "deleteProviderImage",
      MODAL_CLEANUP_REQUEST_DEADLINE_MS,
  ...
  async createSandbox(
    ...
    try {
      const result = await this.postJson(
        request.sandboxExecution?.profile === "docker-v1"
          ? this.endpointUrl("api-create-sandbox-v2")
        ...
        MODAL_SANDBOX_START_REQUEST_DEADLINE_MS,
        {
          ...
          ...(request.sandboxExecution?.profile === "docker-v1"
            ? { allocation_name: request.allocationName }
            : {}),
          ...this.executionBody(request.sandboxExecution),
        },
        createSandboxModalResponseSchema,
        correlation,
        request.signal,
        (status) => (httpStatus = status),
        this.allocationValidator(request.sandboxExecution, request)
      );
  ...
  async restoreSandbox(
    ...
    try {
      const result = await this.postJson(
        request.sandboxExecution?.profile === "docker-v1"
          ? this.endpointUrl("api-restore-sandbox-v2")
  ...
  async createImageBuildSandbox(
    ...
    try {
      const result = await this.postJson(
        request.sandboxExecution?.profile === "docker-v1"
          ? this.endpointUrl("api-create-build-sandbox-v2")
