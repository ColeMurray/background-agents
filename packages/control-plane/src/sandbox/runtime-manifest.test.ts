...
describe("sandbox runtime manifest", () => {
  it("drives control-plane provider labels and compatibility floors", () => {
    ...
    expect(MIN_PRESERVATION_RUNTIME_GENERATION).toBeGreaterThanOrEqual(
      MIN_COMPATIBLE_RUNTIME_GENERATION
    );
    expect(MIN_PRESERVATION_RUNTIME_GENERATION).toBeLessThanOrEqual(SANDBOX_RUNTIME_GENERATION);
