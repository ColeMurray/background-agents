...
async function handleListRepos(
  ...
  // Check KV cache first (provider-specific key so Bitbucket and GitHub don't share cache)
  try {
    const cached = await env.REPOS_CACHE.get<CachedReposList>(CACHE_KEY, "json");
    if (cached) {
      return json({
        repos: cached.repos,
        cached: true,
  ...
  if (vcsProvider === "bitbucket") {
    ...
    try {
      ...
      try {
        await env.REPOS_CACHE.put(CACHE_KEY, JSON.stringify(cacheData), {
