...
ALLOCATION = {"allocation_name": "allocation-1"}
...
def test_versioned_launch_rejects_malformed_execution(execution):
    with pytest.raises(ValidationError):
        CreateSandboxV2Request.model_validate(
            {**BASE, **ALLOCATION, "sandbox_execution": execution}
        )


def test_versioned_launch_requires_explicit_execution_and_rejects_unknown_keys():
    with pytest.raises(ValidationError):
        CreateSandboxV2Request.model_validate({**BASE, **ALLOCATION})
    with pytest.raises(ValidationError):
        CreateSandboxV2Request.model_validate(
            {**BASE, **ALLOCATION, "sandbox_execution": DOCKER, "docker": True}
        )
    assert (
        CreateSandboxV2Request.model_validate(
            {**BASE, **ALLOCATION, "sandbox_execution": DOCKER}
...
def test_versioned_launch_requires_allocation_name():
    with pytest.raises(ValidationError):
        CreateSandboxV2Request.model_validate({**BASE, "sandbox_execution": DOCKER})
...
def test_legacy_restore_must_not_silently_overwrite_nested_docker_intent():
    ...
    assert (
        RestoreSandboxV2Request.model_validate(
            {**body, **ALLOCATION, "sandbox_execution": DOCKER}
        ).sandbox_execution.profile
        == "docker-v1"
    )
