...
describe("repo helpers", () => {
  const repos = ["agustind/andromeda-website", "agustind/botto", "acme/api"];

  it("matches any part of the name, case-insensitively", () => {
    expect(repoChoices(repos, "ANDRO").map((choice) => choice.value)).toEqual([
      "agustind/andromeda-website",
    ]);
    expect(repoChoices(repos, "")).toHaveLength(3);
  });

  it("resolves exact names only", () => {
    expect(findRepo(repos, "Agustind/Botto")).toBe("agustind/botto");
    expect(findRepo(repos, "botto")).toBeUndefined();
  });
});
