  ...
  return withReaction(
    ...
    async () => {
      ...
      await closeOutSupersededHeadStatus(
        ...
        // Present only on `synchronize`; this handler also serves `review_requested`, which has none.
        "before" in payload && typeof payload.before === "string" ? payload.before : undefined
      );
