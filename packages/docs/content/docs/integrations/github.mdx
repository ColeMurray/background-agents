...
There are no GitHub slash commands. Use auto-review or `@mention` comments. To ask for a review on demand, mention the bot with a review request, for example `@my-app[bot] review this PR`.

The entry points differ in where the session starts and what it can push:

```mermaid
flowchart TB
    accTitle: GitHub entry points and what each one posts back
