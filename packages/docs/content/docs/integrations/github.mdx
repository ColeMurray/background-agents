...
The GitHub bot reviews new pull requests, answers when you mention it in a PR, and can feed review feedback back into the session that created the PR.
...
3. For analysis or a reply, mention the bot in a PR comment:

   ```text
   @my-app[bot] can you explain why the checkout test is failing?
   ```

4. For line-specific discussion, mention the bot in an inline review comment.
5. Watch for the eyes reaction, which means the bot accepted the request.
6. Open the web app to watch the full session.
...
There are no GitHub slash commands, and the bot cannot be requested through the PR reviewer picker. Use auto-review or `@mention` comments.

The three entry points differ in where the session starts and what it can push:
...
- The author is allowed. A human author needs write permission on the pull request. A bot author is accepted only for reviews: the app's own reviews when **Open Inspect reviews** is on, and third-party bots listed exactly in **Exact third-party review bots**. Bot-authored top-level comments are never eligible.
...
| Open Inspect reviews          | "Allow reviews from the configured Open Inspect App, regardless of workflow."                | On      |
...
| `own_reviews_disabled`                             | The app's own review while **Open Inspect reviews** is off          |
...
OpenInspect can sign agent-created commits with one deployment-wide OpenSSH Ed25519 key, configured under Settings › Integrations › GitHub › Commit signing. The commit author stays the prompting user (or OpenInspect when unavailable), the committer and signer is a dedicated signing account, branches are pushed by the GitHub App, and the PR author is the prompting user's OAuth identity when available. The signature attests to the deployment, not to the attributed user, and GitHub vigilant mode shows such commits as **Partially verified**. Private keys never reach a sandbox. Setup, rotation, and limitations are covered in [Source control](/configure/source-control).

## Admin and safety notes

**Access boundaries.** Repository access is deployment-scoped through the GitHub App installation; install the App only on intended repositories and use **Repository Scope** as an extra filter. The same App handles OAuth and repository access, and its credentials and webhook secrets stay server-side. By default trigger access requires write, maintain, or admin permission; **Only specific users** replaces that check with the list.
...
**Prompt safety.** Code-review prompts wrap the PR title, author, branches, and description as untrusted; comment prompts wrap the triggering comment. Review-thread file and diff context, and anything the agent reads from GitHub later, are not separately transformed. Webhooks are verified, and duplicate deliveries are deduplicated so retries do not create duplicate sessions.
