...
```mermaid
sequenceDiagram
    accTitle: Sequence of a Slack request and its thread replies
    participant You
    participant Slack
...
In a channel thread, a follow-up's images reach the agent only when that follow-up mentions the bot. A follow-up without a mention still forwards its text, but not its images. In a DM, no mention is needed.
