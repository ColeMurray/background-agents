...
Under **Repository Configuration**, select exactly one repository. The form says "Repository-scoped triggers need exactly one repository." Environments cannot be targeted, and multi-repository fan-out is not available for event triggers.
