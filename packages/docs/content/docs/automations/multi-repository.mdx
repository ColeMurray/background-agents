...
```mermaid
flowchart TB
    accTitle: How one automation firing fans out to one session per repository
...
Multi-repository firings count toward the same three-strike [auto-pause](/automations#auto-pause-after-failures) as single-repository ones, with these rules for multiple targets:
...
**Trigger Now** fires across the full current selection, one session per target. There is no "re-run failed repositories only" action; a manual trigger always starts a fresh firing across the whole selection. The general Trigger Now rules (who it runs as, when it is rejected, and how it affects the schedule) are in [Schedules](/automations/schedules#trigger-now).
