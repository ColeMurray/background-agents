...
2. The control plane pushes the branch with the GitHub App's push credentials. If the session has no named branch yet, it generates one from the session name.
