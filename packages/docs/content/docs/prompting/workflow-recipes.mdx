...
**Context you provide:** the screenshot, the viewport, and the component. **The agent has:** `agent-browser` with headless Chromium in the sandbox, and can start the dev server (use tunnel ports if you also want to open it yourself; see [sandbox tools](/sessions/sandbox-tools)).
