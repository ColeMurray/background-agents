...
- the session URL (use **Copy link** from the More menu above the composer, or the Session actions menu on phones; it copies `/session/<id>`),
...
2. Fix `.openinspect/setup.sh`. Common causes are commands that do not exist in the sandbox (Debian Linux with Node.js, Python, and common dev tools) and builds that exceed the configured build timeout (30 minutes by default; see [Sandbox settings](/configure/sandbox-settings)).
