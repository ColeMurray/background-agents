...
```mermaid
sequenceDiagram
    accTitle: How follow-ups queue behind a running turn and how Stop interrupts it
    actor You
...
While a turn runs the composer placeholder changes to "Add a follow-up..." and the send button reads "Queue". Queued prompts stack above the composer in run order, each row marked with a clock icon. Each row has a "Remove queued prompt" button if you have the `sessions.lifecycle` permission. Removing a prompt puts its text back in the composer so you can edit it and send it again.
...
The web app has no cancel control; in practice a session is cancelled only when a parent agent cancels it with `cancel-child`, and nested children are cancelled with it by default. For a session you started yourself, stop the turn and archive the session instead. See [child sessions](/sessions/child-sessions).
...
<Accordions>
  ...
  <Accordion title="Send is disabled">
    ...
    and tells you to raise or remove the limit), or the selected model is unavailable. Viewers do not see the
    composer at all; if there is no composer on the page, your role is Viewer.
