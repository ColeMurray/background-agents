...
| ---------------- | -------------------------------------------------------------- | --------------------------------------- |
| Shared skill     | Stores reusable instructions and optional supporting files     | Everyone using the OpenInspect instance |
...
Shared skills are installation-wide. Any signed-in user can create, edit, enable, disable, assign, or delete them, so coordinate changes with the other users of your instance.
...
Automations and integrations without a skill selector use All applicable. Child sessions created by an agent inherit the parent's exact set of skills.

There is no separate installation step. Before the agent starts, OpenInspect validates and installs the selected skills ("Installing skills" in the session header). If selected content cannot be fetched, validated, or installed, the session fails to start.
