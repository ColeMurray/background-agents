...
The sandbox is Debian Linux with Node.js 22, Python 3.12, Bun, git, the GitHub CLI, and build-essential preinstalled, plus npm, pnpm, pip, and uv. Anything else your scripts need, they must install.
...
The runtime waits up to 30 seconds for fresh URLs before running `start.sh`. If they are not resolved in time, `start.sh` runs without a fresh file. The session UI still receives the URLs separately and shows them in the Tunnel URLs sidebar section. The file is not written in build mode or when no tunnel ports are configured.

## Preinstalling local MCP packages

Local [MCP servers](/configure/mcp-servers) launched with `npx` are prepared at session start. A package version that is already installed globally with intact executable links is reused, including after a snapshot restore; only missing or mismatched packages are installed. Unversioned packages and tags such as `latest` are refreshed once per sandbox boot.

To remove that install from first-session startup, pin the same version in both the MCP command and `setup.sh`, then rebuild the prebuilt image:
...
with the MCP command configured as `["npx", "-y", "@example/mcp-server@1.2.3"]`. MCP settings are not baked into images on their own; changing the pinned version causes an install until the image is rebuilt with it.
...
- **Do not write long-lived secrets to disk in `setup.sh`.** Anything the script persists (an `.npmrc`, a `.env` file, a downloaded credential) is captured in the prebuilt image and re-served to every session that boots from it, even after you rotate the secret. Read secrets from the environment at runtime instead. See [Secrets](/configure/secrets).
