...
def _patch_paths(
    ...
    with patch("sandbox_runtime.core_services.Path") as MockPath:
...
class TestInstallTools:
    ...
    def test_does_not_claim_a_divergent_existing_lockfile(self, tmp_path):
        ...
        installed = CoreAgentServices._stage_opencode_deps(deps_cache, opencode_dir)
...
class TestResolveGlobalConfigDir:
    ...
    def test_uses_opencode_config_dir_override(self, tmp_path, monkeypatch):
        ...
        assert CoreAgentServices._resolve_opencode_global_config_dir() == tmp_path / "custom"
    ...
    def test_uses_xdg_config_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path / "xdg"))
        assert (
            CoreAgentServices._resolve_opencode_global_config_dir() == tmp_path / "xdg" / "opencode"
        )
    ...
    def test_falls_back_to_home_config(self, tmp_path, monkeypatch):
        ...
        assert (
            CoreAgentServices._resolve_opencode_global_config_dir()
