...
async def test_stop_execution_bounds_hung_stop_request() -> None:
    ...
    client = SimpleNamespace(
        ...
        wait_until_session_idle=AsyncMock(),
    )
    ...
    client.wait_until_session_idle.assert_not_awaited()
...
async def test_stop_execution_requests_stop_then_confirms_idle() -> None:
    client = SimpleNamespace(
        ...
        wait_until_session_idle=AsyncMock(return_value=True),
    )
    ...
    client.wait_until_session_idle.assert_awaited_once()
    assert client.wait_until_session_idle.await_args.args == (SESSION_ID,)
    remaining = client.wait_until_session_idle.await_args.kwargs["timeout_seconds"]
