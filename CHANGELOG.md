...
sessions. The header's status popover names the phase a failed boot died in, and a script whose
failure ends the boot shows its last lines of output there. Long setup scripts no longer trip the
four-minute connect timeout; a boot may take up to `SANDBOX_BOOT_TIMEOUT_MS` (30 minutes by
default). See [How Open-Inspect Works](docs/HOW_IT_WORKS.md#fresh-start-no-snapshot).
