...
| ------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Lifecycle alarms check connecting, heartbeat, boot, and inactivity limits, but not provider lifetime.   | A healthy, busy Sandbox can reach provider expiry without a final preservation operation. | [Lifecycle alarm handling](https://github.com/ColeMurray/background-agents/blob/222f1c002f1ba9ff7624090e1424b7e556b772f6/packages/control-plane/src/sandbox/lifecycle/manager.ts#L1485)                                                                                                                                                                         |
...
| Inactivity can announce “snapshot saved” without a successful result.                                   | Users can receive an incorrect preservation assurance.                                    | [Inactivity stop](https://github.com/ColeMurray/background-agents/blob/222f1c002f1ba9ff7624090e1424b7e556b772f6/packages/control-plane/src/sandbox/lifecycle/manager.ts#L1649)                                                                                                                                                                                  |
...
1. Begin final preservation early enough to finish before a known provider cutoff under the tested
   operating budget.
...
5. Never report final preservation success or silently restore an older recovery point after final
   preservation failed.
...
- Reconstructing missing model conversation history from the Session Event Stream: this is separate
  from filesystem preservation and
...
| Provider         | Current implementation                                                                                                                                         | Final preservation and retirement                                                                                                                                                                | Deadline source                                                                                                                                                             |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| **E2B**          | Session snapshots intentionally unsupported. Stop pauses; resume reconnects the retained object and can change its timeout.                                    | Stop prompt → explicit pause → verify paused/retained → commit retained-object receipt. Never use the destructive kill branch as successful preservation.                                        | Parse provider detail `endAt` after create, connect, and timeout mutation. A reconnect is not itself evidence of a renewed lease.                                           |
...
metadata, and only then start the runtime. Checkpoints outlive their source; preservation polls
until the exact artifact is ready before publication.
...
| Capture or retained-state preservation | 300 seconds    |
...
version, and last successful preservation time. Do not expose locators or credentials through the
public Session projection. Retained E2B/Daytona objects are mutable on resume: their timestamps are
not promises of an immutable historical rollback point.
...
with the identical generation. A same-generation reconnect does not clear the runtime's preservation
...
3. Send the existing authenticated sandbox socket:
   ...
   const command = {
     ...
     operationId,
     generation,
     messageId, // optional for idle preservation
     stopByMs,
   };
   ```
...
5. Ready artifact/retained-state proof; NotFound is not preservation; source retirement only after
   receipt publication (or atomically with preservation).
