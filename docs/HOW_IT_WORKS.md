...
2. **Git sync**: Clones your repository using deployment-level SCM clone credentials
...
1. Agent pushes the branch using deployment-level SCM app credentials
...
The system uses a shared deployment-level SCM integration for all git operations. This means:

- Any user can access any repository the deployment-level app or consumer can access
...
| Token              | Purpose                              | Scope                            |
| ------------------ | ------------------------------------ | -------------------------------- |
| App Token          | Clone repos, push commits            | All repos where the app can access |
...
2. **Limit provider scope**: Only install or authorize on repositories you want accessible
3. **Use provider-side repository scoping**: Don't grant deployment-wide access unless you intend it
