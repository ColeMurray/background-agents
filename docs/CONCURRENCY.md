...
| F6  | `triggerSnapshot`                                                                   | sandbox read, `snapshotting` write, image stamp, status restore                                                | `beginSnapshot()` returning the generation / `completeSnapshot(generation, imageId)`. The folded operation must carry the identity the snapshot was taken of, not just the image: `completeSnapshot(imageId)` alone would reintroduce the stamp race #1764 closed |
