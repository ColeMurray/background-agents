...
The session core reaches storage in three ways. The first is not an await at all, which is what
makes it a guard; the other two are the await classes, and they differ on this host:

- **Synchronous session SQL — not an await.** The per-session store is the Durable Object's SQLite
  (`sql.exec`), read and written through the repositories in `src/session/*-repository.ts`. These
  calls return values, not promises: a read and a write with no `await` between them are atomic with
  respect to every other event, and a repository call is never a boundary. One method is the
  exception — `SandboxRepository.updateSandboxAccess` awaits encryption before it executes, and is
  an ordinary await, below. (`storeTtyd` mints a JWT before calling it, but that is a lifecycle
  manager method, not a repository one.)
...
- Alarm-storage awaits interleave too. There is no gate: an await on the host alarm index
  ...
  after its stop deadline is armed) is not safe on Node. The deadline itself is delivered by one
  host-wide clock (`src/node/host-alarm-clock.ts`) that claims the session's row for the duration of
  a delivery, so the session reads as disarmed while its handler runs, as the platform's does.
  (#1788 turns that claim into a fenced lease the clock can take back; nothing in this document
...
Session SQL is synchronous on both hosts, over different engines: the Durable Object's `sql.exec`
there, `node:sqlite` here, one file per session, opened by one process. The property the
same-continuation guard rests on is the same either way. The registry is the ownership boundary: a
second process, or a second registry in the same process, must never open a session file that
another has resident. Deploy the Node host as one process per session set; there is no leader
election and none is needed while this holds.
...
(`updateSandboxAuthTokenHash` applies only while the reserved identity is still the row's and it is
still `spawning`). The D1 session index uses the same shape with an `updated_at` fence. When one row
is reused across attempts, as the singleton sandbox row is, the condition has to name the attempt
and not just the status: `transitionSandboxStatus` takes a _generation_ — the sandbox's logical id
and the `created_at` its reservation stamped — so a status a newer attempt happens to have returned
to is not mistaken for the caller's own (#1764).
...
- [ ] Name every value captured before the await that a write after it depends on. Repository reads,
      and equally plain locals: a `now` taken at the top of the handler, a status or id read off an
      event. The documented `context.now` overwrite is a local, not a repository read.
- [ ] For each, either capture it after the await (same continuation as the write), or make the
...
Every read → `await` → dependent-write sequence in the eight files the audit covers, classified:
...
A (c) row names the fix that closes it, and stays (c) until that fix is on `main`. #1761, #1762,
#1764 and #1767 have since merged: the rows they closed are (a) below and name the PR that closed
them, so the table reads as the code is. Five rows are still (c); their fixes are not yet opened.
...
| ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enqueuePromptCore` promptability check → `await fingerprintWebPrompt` | session status read → hash await → idempotency read, capacity, insert, `transition("active")`                                              | (a)       | A cancel or archive landing during the hash once left the insert and the `active` transition unguarded: the closed session accepted a message and flipped back to `active`. #1761 moved `assertPromptableSession` after the await, into the insert's continuation, where the idempotency lookup, capacity check, and insert already were.                                   |
...
| `processMessageQueue`, no-socket path                                  | next-pending read → `await getProviderAuthenticationError` → `sandbox_spawning` broadcast, `spawnSandbox()` submitted                      | (a)       | No claim exists on this path — `spawnSandbox` checks sandbox state, not session liveness — so a cancel during the await once left the continuation starting a sandbox for a closed session. #1767 added `isPromptStillDispatchable(message.id)`, which re-reads session liveness and the prompt's `pending` status in the spawn's continuation and pumps the queue instead. |
| `processMessageQueue` after send                                       | send fails → `updateMessageToPending`; send ok → `updateLastActivity(now)`                                                                 | (a) / (c) | `updateMessageToPending` is `WHERE status='processing'`. `now` was read before the auth await and `updateSandboxLastActivity` is an unconditional assignment: a heartbeat that landed meanwhile is moved backwards and the inactivity window shortened. Fix: the write becomes `MAX(last_activity, ?)`, or `now` is taken after the await.                                  |
...
| `handleExecutionComplete` after completion         | completion → `await projectTerminalMessage` (D1) → broadcasts → `await reconcileAfterExecution` → `transition(completed \| failed \| active)` | (a)   | An archive (allowed: no unfinished messages remain) or cancel landing during the projection await was once overwritten: reconcile derived `completed`/`failed` from message state and `transition` wrote it over `archived`/`cancelled`. #1762 made `reconcileAfterExecution` return early on a session that is no longer promptable, reading that status in the same turn as the transition it guards. |
...
| `handleExecutionComplete` tail                     | `updateLastActivity(context.now)` → `await scheduleInactivityCheck` → `processMessageQueue`                                                   | (c)   | `context.now` was read before the projection and reconcile awaits; the same non-monotonic overwrite as the queue's, and `context.now` is a plain local rather than a repository read. Same fix. The queue re-reads everything.                                                                                                                                                                          |
...
| `reconcileAfterExecution`, `reconcileAfterQueueRemoval` | message-count reads → `transition`                                                             | (a)   | The reads are fresh, but the decision once ignored `cancelled`/`archived`, so a reconcile reached after an await (row above) moved a closed session. #1762 gave both an `isSessionClosed()` check in the transition's own turn; `transition` still writes whatever it is given, so the guard belongs to the caller that derives a status from message state. |
...
| `applyLiveSnapshot` → `applyPullRequestSnapshot` | `await` D1 upsert → re-read artifact → staleness check → `updateArtifact`                          | (a)   | Two layers: the service wrapper (`pull-request-service.ts:605`) and the shared operation it delegates to (`pull-request-snapshot-apply.ts:41`), which `pull-request-refresh.ts:127` also calls directly. The apply-time re-read is the pattern, and it is in the shared operation, so both entry points inherit it: authority first, then a guarded mirror write in the re-read's continuation. |
...
| `reserveSpawnIdentity`                                                                         | reservation write → `await` alarm storage → `await hashToken` → `updateSandboxAuthTokenHash(expectedSandboxId)`                           | (a)   | The hash applied `WHERE modal_sandbox_id = ?` (#1606), which excludes a newer reservation but not a cancel of this one: the cancel handler sets the sandbox `stopped` without changing its id, so the hash went live and the provider was called for a closed session. #1764 added `AND status = 'spawning'` to `updateSandboxAuthTokenHash`; a stopped reservation now takes the existing superseded exit without failure writes.                                                                                                                                                                                   |
...
| access publication (`storeCodeServer`, `storeVnc`, `storeTtyd`, `storeAndBroadcastTunnelUrls`) | provider await → (`await mintJwt`) → `updateSandboxAccess` (`await encrypt` inside) / `updateSandboxTunnelUrls` → unconditional row write | (c)   | A termination during the provider, JWT, or encryption await (alarm, cancel, unresponsive-sandbox path) clears access and retires the row; the late write republishes a URL and credential onto a dead sandbox, which the session snapshot then serves. A status-only condition on the attempt does not cover it: the bridge may legitimately have set `ready` meanwhile. Fix: the repository writes apply only while the row is not in a dead status (`stopped`, `stale`, `failed`), after the encryption await. Identity is stable within the attempt under the in-memory flag, so no identity condition is needed. |
| `finishProviderStartup`                                                                        | provider await → socket / status read → `updateSandboxStatus("connecting")`                                                               | (a)   | A provider call longer than the connecting timeout lets the alarm mark the row `failed` (or a connected-then-dead bridge mark it `stale`); the late return overwrote that with `connecting` and broadcast it. #1764 made it `transitionSandboxStatus(generation, "spawning", "connecting")` and act on the result.                                                                                                                                                                                                                                                                                                   |
| failure branches (`doSpawn` catch, restore failure, `resumeSandbox` catch)                     | provider await → `incrementCircuitBreakerFailure` → `updateSandboxStatus("failed")` → `reportSandboxError`                                | (a)   | A bridge that connected during the provider call set the row `ready` and attached its socket; a provider error after that (a post-create timeout) marked the working sandbox `failed`, which the alarm treats as dead and stops monitoring, and persisted a spawn error the snapshot serves. #1764 routed all three through `failAttempt`, which transitions only from the attempt's own `spawning`/`connecting`. The circuit-breaker increment stays unconditional: it counts the attempt. The retry-from-base path re-reserves the identity unconditionally and supersedes such a bridge; recorded, not changed.   |
| `triggerSnapshot` status restore                                                               | sandbox read → `snapshotting` → `await takeSnapshot` → `updateSandboxStatus(previousStatus)`                                              | (a)   | A cancel, heartbeat-stale alarm, or unresponsive-sandbox termination during the snapshot set `stopped`/`stale`, cleared access, and detached the socket; the snapshot's return restored the pre-snapshot `ready`, so the spawn decision waited for a reconnect that cannot come. #1764 made the restore `transitionSandboxStatus(generation, "snapshotting", previousStatus)`.                                                                                                                                                                                                                                       |
| `triggerSnapshot` image stamp                                                                  | sandbox read → `await takeSnapshot` → `updateSandboxSnapshotImageId(sandbox.id, …)`                                                       | (a)   | The stamp was keyed by row id, and spawn reservations reuse the one row: a sandbox terminated and replaced during the snapshot handed its image to the replacement as its restore image. #1764 replaced it with `recordSandboxSnapshot(generation.sandboxId, …)`, which applies `WHERE modal_sandbox_id IS ?` on the identity captured before the await.                                                                                                                                                                                                                                                             |
...
| `attachSandbox` | activity + heartbeat writes → `await scheduleInactivityCheck` (alarm storage) → adopt socket, `onSandboxConnected`, `updateSandboxStatus("ready")` | (c)   | Safe on Cloudflare only because the await is alarm storage. On Node a cancel, a termination, or a replacement reservation landing there is followed by an obsolete socket being adopted and `ready` written over `stopped`. Fix: re-read session liveness, reconnect-blocked status, and the presented identity in the adoption's continuation, and close the socket instead. Adoption is an admission protocol, not a repository fold. |

Merged since the first revision, each behavior-preserving on Cloudflare outside the interleaving it
closes:

- #1761: `enqueuePromptCore` re-checks promptability after the fingerprint await.
- #1762: `reconcileAfterExecution` and `reconcileAfterQueueRemoval` leave a session that is no
  longer live.
- #1764: `SandboxStorage.transitionSandboxStatus(generation, from, to)` and
  `recordSandboxSnapshot(sandboxId, …)` replace the unconditional status writes and the image stamp
  that follow a provider await; the hash publication also requires `spawning`.
- #1767: the no-socket dispatch path re-reads session liveness and the prompt's status before
  submitting a spawn.

Five (c) rows remain, in three classes; none has a PR open yet:

- Non-monotonic activity (`processMessageQueue` after send, `handleExecutionComplete` tail):
  `updateSandboxLastActivity` becomes `MAX(last_activity, ?)`, or the timestamp is taken in the
  final continuation.
- Unfenced projections (`syncSessionMetrics`): `updateMetrics` carries the transition's `updated_at`
  and applies `WHERE updated_at <= ?`, the fence `updateStatus` already uses.
- Publication onto a dead row (access and tunnel publication; `attachSandbox` adoption): the
  repository writes apply only while the row is not `stopped`/`stale`/`failed`, after the encryption
  await; `attachSandbox` re-reads liveness, reconnect-blocked status, and the presented identity in
  the adoption's continuation and closes the socket instead.
...
| F6  | `triggerSnapshot`                                                                   | sandbox read, `snapshotting` write, image stamp, status restore                                                | `beginSnapshot()` returning a token of the identity _and_ the status it displaced / `completeSnapshot(token, imageId)`. Both halves are read before the provider await (`manager.ts:1163-1167`) and both are needed after it: the stamp is conditional on the identity, and the restore passes the displaced status to `transitionSandboxStatus(generation, "snapshotting", previousStatus)` (`manager.ts:1235`). A token of the generation alone cannot express the restore, and `completeSnapshot(imageId)` alone would reintroduce the stamp race #1764 closed |
| F7  | `reserveSpawnIdentity`                                                              | reservation, hash publication                                                                                  | Already two-phase by design (#1606), and conditional on identity plus `spawning` since #1764; keep                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| F10 | `applyPullRequestSnapshot`                                                          | D1 upsert, artifact re-read, staleness check, artifact update                                                  | Already the conditional pattern; becomes one operation under async repositories. `PullRequestService.applyLiveSnapshot` (`pull-request-service.ts:605`) is a wrapper over it, not a second fold; `pull-request-refresh.ts:127` calls the shared operation directly, so the guard has to live here rather than in either caller                                                                                                                                                                                                                                    |
| F11 | `syncSessionMetrics`                                                                | message count, active duration, artifact list, session cost → one index write                                  | Already one write; the open fix adds the `updated_at` fence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
