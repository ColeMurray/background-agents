...
that session, issues the token only to sessions the GitHub bot created (any other session's sandbox
gets 403), and returns `Cache-Control: no-store`. Only the review POST uses this credential; other
