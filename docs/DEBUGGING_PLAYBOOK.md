...
| `repo_image.build_complete_received`         | info  | `build_id`, `provider`, `provider_session_id`, `base_sha`, `trace_id`             | Provider-session runtime reported success; control-plane finalization begins   |
| `repo_image.<provider>_finalize_start`       | info  | `build_id`, `provider_session_id`, `trace_id`, `request_id`                       | Control plane started snapshot/checkpoint finalization for Vercel/OpenComputer |
| `repo_image.build_complete`                  | info  | `build_id`, `provider`, `provider_image_id`, `replaced_image_id`, `trace_id`      | Repo image artifact marked ready                                               |
| `repo_image.build_superseded`                | info  | `build_id`, `provider`, `provider_image_id`, `provider_session_id`, `trace_id`    | Build completed after a newer ready image already existed                      |
| `repo_image.build_failed`                    | info  | `build_id`, `provider`, `error_message`, `trace_id`, `request_id`                 | Runtime reported a failed repo image build                                     |
...
| `repo_image.<provider>_finalize_error`       | error | `build_id`, `provider_session_id`, `provider_image_id`, `error`, `duration_ms`    | Provider-session finalization threw before or after creating an image          |
| `repo_image.<provider>_finalize_not_applied` | warn  | `build_id`, `provider_session_id`, `provider_image_id`, `duration_ms`, `trace_id` | Finalization succeeded after the build row stopped accepting completion        |
