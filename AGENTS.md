...
- `discord-bot` — Discord `/task` slash command → coding sessions
...
```
@open-inspect/shared  ←  control-plane, web, slack-bot, github-bot, linear-bot, discord-bot
```
...
| `discord-bot`   | TypeScript / CF Workers + Hono     | Discord `/task` interactions handler                        |
...
- **web, slack-bot, linear-bot, discord-bot**: co-located `src/**/*.test.ts`
