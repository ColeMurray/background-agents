...
output "verification_commands" {
  ...
  value       = <<-EOF
    ...
    ${local.use_modal_backend ? "curl ${module.modal_app[0].api_health_url}" : "# ${var.sandbox_provider} sandboxes use the provider API directly — no health endpoint to check"}
